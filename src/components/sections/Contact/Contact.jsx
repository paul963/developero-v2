...
const Contact = () => {
  ...
  const isFormValid = formData.name.trim() !== "" && formData.email.trim() !== "" && formData.phone.trim() !== "" && formData.service !== "";
  ...
  return (
    <section className="contact section-py" id="contact">
      <div className="container">
        ...
        <p className="text-center description">Ai întrebări despre serviciile noastre sau vrei o ofertă? Trimite-ne un mesaj și revenim cu un răspuns.</p>
        <div className="form-wrapper bg-dark">
          <form className="form" onSubmit={handleSubmit}>
            <div className="grid grid-2">
              ...
              <input name="company" placeholder="Denumire Companie" value={formData.company} onChange={handleChange} />
            </div>
            <div className="grid grid-2">
              <input type="email" name="email" placeholder="Email" required value={formData.email} onChange={handleChange} />
              <input type="tel" name="phone" placeholder="Telefon" required value={formData.phone} onChange={handleChange} />
            </div>
            <select name="service" required ref={selectRef} value={formData.service} onChange={handleChange}>
